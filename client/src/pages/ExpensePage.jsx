  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/stats/transactions', {
        ...
        params: { type: 'expense', page, search }
      });
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: 40, textAlign: 'center' }}>
          <div className="muted" style={{ marginBottom: 16 }}>No expense transactions found</div>
          <button onClick={handleAddNew} className="btn">Add your first expense</button>
        </div>
      ) : (
              ...
              <tbody>
                {transactions.map((transaction) => (
                  <tr key={transaction._id} style={{ borderBottom: '1px solid rgba(148,163,184,0.1)' }}>
                    ...
                    <td style={{ padding: 16, textAlign: 'right', fontWeight: 600, color: '#ef4444' }}>
